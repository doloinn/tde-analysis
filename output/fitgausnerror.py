...
for i, r in enumerate(eldf['in radius'].unique()):
    for j, v in enumerate(eldf['in v mag'].unique()):
        ...
        for c, ba in enumerate(magdf['in b/a'].unique()):
            ...
            plt.hist(magdf_detected['source_g_mag'], bins=bins, color='C0', normed=1, edgecolor='black')
            histout = plt.hist(magdf_detected.loc[magdf['in b/a'] == ba]['out found_mag'], normed=1, color='C2', edgecolor='black', alpha=0.6, label='$b/a = %s$' %ba)
            ...
            try:
                ...
                shift.append(popt[0])
                dshift.append(popt[1])
                print(pstd)
                print('%s, %s, %s' %(r, v, ba), shift[-1], dshift[-1])
            ...
            fig.canvas.set_window_title(" %s, %s, %s" %(r, v, ba))
