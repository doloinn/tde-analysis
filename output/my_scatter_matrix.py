# David Lynn
# Modified Pandas' scatterhist to Plot 2D histograms instead of scatter
# plots
...
def scatter_matrix(frame, allframe, alpha=0.5, figsize=None, ax=None, grid=False,
                   diagonal='hist', marker='.', density_kwds=None,
                   hist_kwds=None, range_padding=0.05, colmap='Blues', **kwds):
    """
    ...
    alldf = allframe._get_numeric_data()
    n = df.columns.size
    naxes = n * n
    ...
    for i, a in zip(lrange(n), df.columns):
        for j, b in zip(lrange(n), df.columns):
            ...
            if i == j:
                values = df[a].values[mask[a].values]
                allvalues = alldf[a].values[allmask[a].values]
                ...
                if diagonal == 'hist':
                    ...
                    bins = 10
                    if len(uni) < 4:
                        bins = np.append(uni, uni[-1] + .1)

                    if not a[:3] == 'out' or not b[:3] == 'out':
                        histout = np.histogram(allvalues, bins=bins, **hist_kwds)
                        ax.bar(np.arange(len(histout[0])) + 0.5, histout[0], width=1, color='C2', edgecolor='black') 

                    histout = np.histogram(values, bins=bins, **hist_kwds)
                    ax.bar(np.arange(len(histout[0])) + 0.5, histout[0], width=1, color='C0', edgecolor='black') 


                    ax.set_xlim([0, len(histout[0])])

                    # ax.set_xticks([0.5 + k for k, _ in enumerate(histout[0])])
                    # ax.set_xticklabels(['%.3g' % k for k in histout[1]])

                    if type(bins) != int:
                        # print('x here')
                        ax.set_xticks([0.5 + k for k, l in enumerate(histout[0])])
                        ax.set_xticklabels(get_dec(histout[1]))
                    else:
                        ax.set_xticks([k for k, l in enumerate(histout[1])][1:-1])
                        ax.set_xticklabels(get_dec(histout[1])[1:-1])
            ...
            # else:
                ...
                allcommon = (allmask[a] & allmask[b]).values

                uni = [np.sort(df[b][common].unique()), np.sort(df[a][common].unique())]
                # print(uni)
                bins = [10, 10]
                for k, c in enumerate(uni):
                    # print(a, b)
                    if len(c) == 1 or len(c) < 4 and c[0] < c[1]:
                    ...
                    # elif df
                        ...
                        bins[k] = bins[k][::-1]
                # print(bins)
                histout = np.histogram2d(df[a][common], df[b][common], bins=bins[::-1])

                if a[:3] == 'out' or b[:3] == 'out':
                    im = ax.imshow(histout[0], interpolation=None, aspect='auto', origin='lower', cmap=plt.get_cmap('Reds'), extent=[0, len(histout[2]) - 1, 0, len(histout[1]) - 1])
                else:
                    allhistout = np.histogram2d(alldf[a][allcommon], alldf[b][allcommon], bins=bins[::-1])
                    im = ax.imshow(histout[0] / allhistout[0], interpolation=None, aspect='auto', origin='lower', cmap=cmap, vmin=0, vmax=1, extent=[0, len(histout[2]) - 1, 0, len(histout[1]) - 1])
                # print(bins, histout[1:3])
                if type(bins[0]) != int:
                    # print('x here')
                    ax.set_xticks([0.5 + k for k, l in enumerate(histout[0][0, :])])
                    ax.set_xticklabels(get_dec(histout[2]))
                else:
                    ax.set_xticks([k for k, l in enumerate(histout[2])][1:-1])
                    ax.set_xticklabels(get_dec(histout[2])[1:-1])
                if type(bins[1]) != int:
                    # print('y here')
                    ax.set_yticks([0.5 + k for k, l in enumerate(histout[0][:, 0])])
                    ax.set_yticklabels(get_dec(histout[1]))
                else:
                    ax.set_yticks([k for k, l in enumerate(histout[1])][1:-1])
                    ax.set_yticklabels(get_dec(histout[1])[1:-1])
                    
                # ax.set_xticks([0.5 + k for k, l in enumerate(histout[0][0, :])])
                # ax.set_xticklabels(['%.3g' % k for k in histout[2]])
                # ax.set_yticks([0.5 + k for k, l in enumerate(histout[0][:, 0])])
                # ax.set_yticklabels(['%.3g' % k for k in histout[1]])
            ...
            if i >= j and j != 0:
                ...
                ax.set_yticklabels([])
                ax.set_ylabel('')
            ax.tick_params(axis='y', direction='in', length=2, left='on', right='on')
                # ax.tick_params(axis='x', direction='inout')
            #     ax.spines["left"].axis.axes.tick_params(direction="inward") 
            # else:
            #     ax.spines["left"].axis.axes.tick_params(direction="outward") 
            if i != n - 1:
                ax.set_xticklabels([])
                ax.set_xlabel('')
            ax.tick_params(axis='x', direction='in', length=2, top='on', bottom='on')
            # else:
            #     ax.tick_params(axis='x', direction='inout', length=4, top='on', bottom='on')
            # else:
