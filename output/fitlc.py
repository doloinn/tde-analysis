...
# Fitting lc
fitlc = [i.dropna() for i in random.sample(lc, 2)]

for i in fitlc:

    lcmags = i['out found_mag'].as_matrix()
    lcfluxes = mag2flux(lcmags)
    lctimes = i['in point time'].as_matrix()

    peakidx = lcfluxes.argmax()


    for j, ofun in enumerate(odrfunctions):
        plt.figure()
        for k in range(3):
            startidx = peakidx + k
            ...
            myoutput = myodr.run()
            myoutput.pprint()
            print(myoutput.res_var)

            fitplotx = np.linspace(fittimes.min(), fittimes.max(), 100)
            fitploty = ofun(myoutput.beta, fitplotx)
