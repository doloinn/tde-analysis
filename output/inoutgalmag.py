...
for i, r in enumerate(eldf['in radius'].unique()):
    for j, v in enumerate(eldf['in v mag'].unique()):
        ...
        for c, ba in enumerate(eldf['in b/a'].unique()):
            ...
            histout = np.histogram(magdf_detected.loc[eldf['in b/a'] == ba]['out found_mag'])
            bincens = (histout[1] + (histout[1][1] - histout[1][0]) / 2)[:-1]
            x = bincens
            y = histout[0]
            n = len(x)                          #the number of data
            mean = sum(x*y)/n                   #note this correction
            sigma = sum(y*(x-mean)**2)/n        #note this correction
            popt, pcov = curve_fit(gaus, x, y, absolute_sigma=True, sigma=np.sqrt(y), p0=[1, mean, sigma])
            xfun = np.linspace(min(uni), max(uni), 1000)
            plt.plot(xfun, gaus(xfun, *popt))

            plt.title("Radius $= %s''$, v = %s" %(r, v))
        # plt.title('r = %s, b/a = %s' %(r, ba))
        # plt.title('r = %s' %r)
        # plt.legend(loc='best')
